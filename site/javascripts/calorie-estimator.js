...
function calculateIngredientCalories(ingredientText) {
  ...
  if (unit && UNIT_CONVERSIONS[unit]) {
    conversionFactor = UNIT_CONVERSIONS[unit];
  } else {
    // Try to infer from ingredient's default unit
    if (ingredient.data.unit === 'clove' || ingredient.data.unit === '1') {
      conversionFactor = 1;
    } else {
      conversionFactor = 100; // Default to 100g equivalent
    }
  }
  
  // Calculate based on the ingredient's database unit
  let totalGrams = quantity * conversionFactor;
  
  // Adjust for database unit (per 100g or per unit)
  let caloriesPerUnit = ingredient.data.calories;
  let kjPerUnit = ingredient.data.kj;
  
  if (ingredient.data.unit === '100g' || ingredient.data.unit === '100ml') {
    // Database is per 100g/ml, so divide accordingly
    const calories = (totalGrams / 100) * caloriesPerUnit;
    const kj = (totalGrams / 100) * kjPerUnit;
    return { calories, kj, matched: true, text: ingredientText, ingredient: ingredient.name };
  } else if (ingredient.data.unit === '1' || ingredient.data.unit === 'clove') {
    // Database is per unit (e.g., per egg, per clove)
    const calories = quantity * caloriesPerUnit;
    const kj = quantity * kjPerUnit;
    return { calories, kj, matched: true, text: ingredientText, ingredient: ingredient.name };
  }
